...
struct FM : Module {
    ...
    void process(const ProcessArgs& args) override {
        ...
        for (int ch = 0; ch < channels; ch++) {
            ...
            // frequency
            float carrierFreq = arc::dsp::cvToFrequency(inCarrierPitch);
            float modulatorFreq = clamp(carrierFreq * ratio + offset, 20.0f, 20000.0f);
            float outModulatorPitch = arc::dsp::frequencyToCV(modulatorFreq);
...
struct FMWidget : ModuleWidget {
    FMWidget(FM* module) {
        ...
        // knobs and switches
        addParam(createParamCentered<ArcKnob45>(Vec(37.5, 90), module, FM::kRatioParam));
        addParam(createParamCentered<ArcHSwitch>(Vec(37.5, 132), module, FM::kRatioQuantParam));
        addParam(createParamCentered<ArcKnob45>(Vec(37.5, 197), module, FM::kOffsetParam));

        // row 1
        addParam(createParamCentered<ArcKnob18>(Vec(19.5, 250), module, FM::kRatioCvAmountParam));
        addParam(createParamCentered<ArcKnob18>(Vec(55.5, 250), module, FM::kOffsetCvAmountParam));

        // row 2
        addInput(createInputCentered<ArcPolyPort>(Vec(19.5, 292), module, FM::kRatioCvInput));
        addInput(createInputCentered<ArcPolyPort>(Vec(55.5, 292), module, FM::kOffsetCvInput));

        // row 3
        addInput(createInputCentered<ArcPolyPort>(Vec(19.5, 334), module, FM::kCarrierPitchInput));
        addOutput(
            createOutputCentered<ArcPolyPort>(Vec(55.5, 334), module, FM::kModulatorPitchOutput));

#ifdef FM_DEBUG
        addOutput(createOutputCentered<ArcPolyPort>(Vec(12, 12), module, FM::kDebug1));
        addOutput(createOutputCentered<ArcPolyPort>(Vec(12, 36), module, FM::kDebug2));
        addOutput(createOutputCentered<ArcPolyPort>(Vec(12, 60), module, FM::kDebug3));
        addOutput(createOutputCentered<ArcPolyPort>(Vec(12, 84), module, FM::kDebug4));
#endif
    }
};
